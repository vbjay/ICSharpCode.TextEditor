...
namespace ICSharpCode.TextEditor
{
    ...
    public class Caret : IDisposable
    {
        ...
        private int column;
        private Point currentPos = new Point(x: -1, y: -1);
        ...
        {
            var line = Math.Max(val1: 0, Math.Min(textArea.Document.TotalNumberOfLines - 1, pos.Y));
            var column = Math.Max(val1: 0, pos.X);
        ...
        {
            line = Math.Max(val1: 0, Math.Min(textArea.Document.TotalNumberOfLines - 1, line));
            column = Math.Max(val1: 0, column);
            ...
            while (!caretCreated)
                switch (caretMode)
                {
                    case CaretMode.InsertMode:
                        caretCreated = caretImplementation.Create(width: 2, textArea.TextView.FontHeight);
                        break;
        ...
        private void PaintCaretLine(Graphics g)
        {
            ...
            g.DrawLine(
                BrushRegistry.GetDotPen(caretLineColor.Color),
                currentPos.X,
                y1: 0,
                currentPos.X,
        ...
        private class Win32Caret : CaretImplementation
        {
            ...
            public override bool Create(int width, int height)
            {
                return CreateCaret(textArea.Handle, hBitmap: 0, width, height);
            }
